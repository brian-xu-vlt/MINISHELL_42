...
int		main()
{
	...
	tester("$hellowejfh", "$:$hellowejfh");
	tester("echo bonjour ; | . 2", "WORD:echo-WORD:bonjour-;-|-.-WORD:2");
	tester("echo bonjour | | . 2", "WORD:echo-WORD:bonjour-|-|-.-WORD:2");
	tester("echo bonjour ; ls . bonjour ; ls", "WORD:echo-WORD:bonjour-;-WORD:2");
	tester("cd $HOME/Bureau .", "WORD:cd-$:HOME/Bureau-WORD:.");
	tester("echo -n -n -nnnn -nnnnm . -nnnnm echo -n -nnn", "WORD=echo-WORD:-n-WORD:-n-WORD:-nnnn-WORD:-nnnnm-WORD:.-WORD:-nnnnm-WORD:-n-WORD:-nnn");
	tester("cat /dev/random | head -n 1 | cat -e .", "WORD:cat-WORD:/dev/random-|-WORD:grep-WORD:pr-|-WORD:head-WORD:-n-WORD:5-|-WORD:cat-WORD:-e-WORD:.");
	...
	tester("export \"HI= hi\"", "WORD:echo-\"-WORD:HI=-WORD:hi-\"-");
	tester("export \"HI =hi\"", "WORD:export-\"-WORD:HI-WORD:=hi");
	tester("echo $?", "WORD:echo-$-WORD:?");
	tester("| echo", "|-WORD:echo");
	tester("echo |", "");
	...
	tester("cat < nofile | grep gcc > output // $? 1", "WORD:cat-<-WORD:nofile-|-WORD:grep-WORD:gcc->-WORD:output-WORD://-$-WORD:?-WORD:1");
	...
	tester("test \"num1\" [-eq|-ne|-gt|-lt] \"num2\"", "WORD:test-\"-WORD:num1-\"-WORD:[-q-|-WORD:-ne-|-WORD:-gt-|-WORD:-lt-\"-WORD:num2-\"");
	...
	tester("test -e /etc/passwd && echo \"Le fichier existe\" || echo \"Le fichier n'existe pas\"", "WORD:test-WORD:-e-WORD:/etc/passwd-&&-WORD:echo-\"-WORD:Le-WORD:fichier-WORD:existe-\"-||-\"-WORD:le-WORD:fichier-WORD:n-'-WORD:existe-WORD:pas-\"");
	return (EXIT_SUCCESS);
}
